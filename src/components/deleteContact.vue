    ...
    <div class="addNewOverlay fixed top-0 left-0 w-svw h-svh flex justify-center items-center z-[999]">
        <section class="bg-white p-5 rounded-xl">
            <div class="flex flex-col gap-3">
                <span>are you sure you want to delete <b>{{store.contacts[store.deleteContactIndex].name}}</b> from your contacts?</span><br>
                <span>this action is irreversible</span>
                <div class="newButtons flex gap-3 justify-center">
                    <button class="cancelBUtton py-2 w-10 w-full hover:bg-[#f8f8f8] hover:shadow-md border border-[#d9d9d9] flex justify-center rounded-md" @click="store.deleteContact_ = !store.deleteContact_">
                    ...
                    <button class="submitButton py-2 w-10 w-full hover:bg-[#f8f8f8] hover:shadow-md border border-[#aaa] flex justify-center rounded-md" @click="deleteContact()">
...
<script>
    ...
    export default{
        methods:{
            deleteContact(){
                this.store.deleteElementByIndex(this.store.deleteContactIndex)
            }
        },
        setup(){
